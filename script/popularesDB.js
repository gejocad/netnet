let popularesDB = [
    {id:  0,
        nombre: "Matrix: Recargado",
        imagen: "imagenes/covers/1.jpg",
        coincidencia: "99% de coincidencia",
        año: 2003,
        duracion: "2h 18m",
        descripcion:"Zion, el último reducto de control humano, está en peligro. Las máquinas responsables de Matrix están decididas a destruirlo y la tripulación debe luchar por salvarlo. Segunda parte de la exitosa trilogía de ciencia ficción.",
        elenco: "Keanu Reeves, Laurence Fishburne, Carrie-Anne Moss, Hugo Weaving, Monica Bellucci, Randall Duk Kim, Jada Pinkett Smith.", 
        director: "Andy Wachowski y Larry Wachowski.",
    },
    {
        id:  1,
        nombre: "Bob Esponja: Un héroe al rescate",
        imagen: "imagenes/covers/2.jpg",
        coincidencia: "99% de coincidencia",
        año: 2020,
        duracion: "1h 40m",
        descripción: "Después de que desaparezca su querida mascota, el caracol Gary, Bob Esponja y su fiel amigo Patricio se embarcarán en una aventura épica en la ciudad perdida de Atlantic City, con el objetivo de llevar a Gary a casa.",
        elenco: "Tom Kenny, Bill Fagerbakke, Clancy Brown.",
        director: "Tim Hill.",
        
    },
    {
        id:  2,
        nombre: "El sorprendente hombre araña ",
        imagen: "imagenes/covers/3.jpg",
        coincidencia: "99% de coincidencia",
        año: 2012,
        duracion: "2h 33m",
        descripcion: "La investigación de Peter Parker para resolver la desaparición de sus padres lo coloca en el camino de un choque con el alter ego mortal de un científico conocido como el Lagarto.",
        elenco: "Chris zylka, Annie parisse, C. Thomas Howell, Miles Elliot, Charlie Depew, Max Charles, Kevin Mccorkle, Barbara Eve Harris, Tom Waite.",
        director: "Marc Webb.",
        
    },
    {
        id:  3,
        nombre: "Cómo entrenar a tu dragón",
        imagen: "imagenes/covers/4.jpg",
        coincidencia: "99% de coincidencia",
        año: 2010,
        duracion: "1h 38m",
        descripcion: "La insólita amistad entre el adolescente vikingo Hipo y su fiel compañero dragón, el Furia Nocturna Desdentao, llega a su fin. En esta nueva aventura descubrirán finalmente sus destinos. Mientras el joven gobernante del poblado vikingo de Berk cumple su sueño de crear un mundo utópico en el que los dragones vivan en paz, con ayuda de su compañera Astrid (America Ferrera), el Furia Nocturna será el líder de su propia especie. Además, la aparición de una salvaje y escurridiza dragona, una Furia Luminosa, pondrá a prueba los lazos de su relación como nunca antes. Claro que, cuando la amenaza más oscura a la que se han enfrentado ponga en peligro a sus respectivos clanes, ambos, dragón y vikingo, deberán llevar a cabo decisiones imposibles para salvar a los suyos.",
        elenco: "Jay Baruchel, America Ferrera, F. Murray Abraham.",
        director: "Dean DeBlois.",
        
    }
]

export default popularesDB